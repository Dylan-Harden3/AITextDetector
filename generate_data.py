...
def generate_data(dataset, model, args):
    dataset = load_dataset(
        dataset, split="train", cache_dir=args.cache, trust_remote_code=True
    )["document"]
    ...
    # only keep samples > 250 words
    dataset = [x for x in dataset if len(x.split()) > 250]
    ...
    model, tokenizer = load_model(model, args.cache, token)
    ...
    for batch in range(len(dataset) // args.batch_size):
        ...
        tokenized = tokenizer(
            human_text, return_tensors="pt", padding=True, return_token_type_ids=False
        ).to(device)
    ...
    output_file = (
        args.output_file
        if args.output_file
        else f"{args.model[args.model.index('/'):]}_{args.dataset}.json"
    )
